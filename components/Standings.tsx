...
export const Standings = ({ title, teams, scores, nums }: Props) => {
  return (
    <div className="relative border  border-slate-400 items-center min-w-[320px] flex flex-col  ">
      {/* Title */}
      <div className="flex items-center justify-center w-full p-2 text-lg font-bold text-white bg-red-600 flex-center">
        <p>{title}</p>
      </div>
      ...
      {teams.map((team, index) => (
        <div
          key={index}
          className="flex items-center justify-between w-full p-3 text-lg text-black flex-center"
        >
